...
while ($row = mysqli_fetch_assoc($result)) {
    if ($row['chat_from'] !== $actualUser) {
        ...
        $sql2 = "SELECT * FROM chatrooms WHERE chat_from = '$friend' OR chat_to = '$actualUser'";
        $result2 = mysqli_query($conn, $sql2);
        $row2 = mysqli_fetch_assoc($result2);
        $chatRoomId = $row2['chat_id'];
        $sql3 = "SELECT * FROM chat WHERE chat_room_id = '$chatRoomId' AND chat_room_read = 0 AND chat_room_sender = '$friend' ORDER BY id DESC";
      ...
      $friend = $row['chat_to'];
      $sql2 = "SELECT * FROM chatrooms WHERE chat_to = '$friend' OR chat_from = '$actualUser'";
      $result2 = mysqli_query($conn, $sql2);
      $row2 = mysqli_fetch_assoc($result2);
      $chatRoomId = $row2['chat_id'];
      $sql3 = "SELECT * FROM chat WHERE chat_room_id = '$chatRoomId' AND chat_room_read = 0 AND chat_room_sender = '$friend' ORDER BY id DESC";
